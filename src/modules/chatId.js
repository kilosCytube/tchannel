// src/modules/chatId.js

// Função MD5 (Compactada)
...
function init() {
    console.log("[ChatId] Inicializando gerador de IDs (6 dígitos)...");
        ...
        for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
                ...
                try {
                    ...
                    const usernameClass = classList.find(c => c.startsWith('chat-msg-'));
                    const username = usernameClass ? usernameClass.replace('chat-msg-', '') : 'anon';
                    
                    let msgContent = "";
                    const msgSpan = node.querySelector('span:not(.timestamp):not(.username)');
                    if (msgSpan) msgContent = msgSpan.innerHTML;
    ...
    window.generateMsgId = generateHash;
}
